...
class FindProfessionalsByLocation(models.TransientModel):
    ...
    def _compute_available_professionals(self):
        if not self.project_id or not (self.project_id.latitude and self.project_id.longitude):
    ...
    def action_show_on_map(self):
        """
        Muestra los resultados en un mapa
        """
        return {
            'name': 'Profesionales en Mapa',
            'type': 'ir.actions.act_window',
            'res_model': 'find.professionals.by.location',
            'res_id': self.id,
            'view_mode': 'form',
            'target': 'new',
        }