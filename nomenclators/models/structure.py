...
class Structures(models.Model):
    ...
    def fields_get(self, fields=None, attributes=None):
        res = super(Structures, self).fields_get(fields, attributes=attributes)
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(Structures, self).fields_view_get(view_id=view_id,
                                                           view_type=view_type,
                                                           toolbar=toolbar,
                                                           submenu=submenu)
    ...
    def create(self, vals_list):
        ...
        # Add traces
        model = self.env['ir.model'].search([('model', '=', 'nomenclators.structures')])

        self.env['security.traces'].create({
            'register_time': datetime.utcnow(),
            'user': user.name,
            'model': model.id,
            'description': 'Creación de estructura satisfactoria.'
        })

        return super(Structures, self).create(vals_list)
    ...
    def write(self, vals):
        ...
        self.env['security.traces'].create({
            ...
            'user': user.name,
            'model': model.id,
            'description': 'Edición de estructura satisfactoria.'
        })
        return super(Structures, self).write(vals)
    ...
    def unlink(self):
        ...
        model = self.env['ir.model'].search([('model', '=', 'nomenclators.structures')])
        user = self.env['res.users'].search([('id', '=', int(self.env.uid))])
        msg = 'Eliminación de estructura satisfactoria.'
        if count > 1:
            msg = 'Eliminación de estructuras satisfactoria.'
        self.env['security.traces'].create({
            'register_time': datetime.utcnow(),
            'user': user.name,
            'model': model.id,
        ...
        return super(Structures, self).unlink()