...
class TeachingCategories(models.Model):
    _name = 'nomenclators.teaching_categories'
    _description = 'Categorías de enseñanza'
    _rec_name = 'name'
    ...
    _sql_constraints = [
        ('name_uniq', 'unique(name, company_id)', 'Ya existe una categoría de enseñanza con ese nombre!'),
    ]

    @api.model
    def fields_get(self, fields=None, attributes=None):
        res = super(TeachingCategories, self).fields_get(fields, attributes=attributes)
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(TeachingCategories, self).fields_view_get(view_id=view_id,
                                                    view_type=view_type,
                                                    toolbar=toolbar,
                                                    submenu=submenu)
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        res = super(TeachingCategories, self).search_read(domain, fields, offset, limit, order)
        return res
    ...
    def create(self, vals_list):
        ...
        # Add traces
        model = self.env['ir.model'].search([('model', '=', 'nomenclators.teaching_categories')])

        self.env['security.traces'].create({
            'register_time': datetime.utcnow(),
            'user': user.name,
            'model': model.id,
            'description': 'Creación de categoría de enseñanza satisfactoria.'
        })

        return super(TeachingCategories, self).create(vals_list)
    ...
    def write(self, vals):
        ...
        self.env['security.traces'].create({
            ...
            'user': user.name,
            'model': model.id,
            'description': 'Edición de categoría de enseñanza satisfactoria.'
        })
        return super(TeachingCategories, self).write(vals)
    ...
    def unlink(self):
        ...
        model = self.env['ir.model'].search([('model', '=', 'nomenclators.teaching_categories')])
        user = self.env['res.users'].search([('id', '=', int(self.env.uid))])
        msg = 'Eliminación de categoría de enseñanza satisfactoria.'
        if count > 1:
            msg = 'Eliminación de categorías de enseñanza satisfactoria.'
        self.env['security.traces'].create({
            'register_time': datetime.utcnow(),
            'user': user.name,
            'model': model.id,
        ...
        return super(TeachingCategories, self).unlink()
