...
class LabourSector(models.Model):
    _name = 'nomenclators.labour_sector'
    ...
    _sql_constraints = [
        ('name_uniq', 'unique(name, company_id)', 'Ya existe un sector laboral con ese nombre!'),
    ]

    @api.model
    def fields_get(self, fields=None, attributes=None):
        res = super(LabourSector, self).fields_get(fields, attributes=attributes)
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(LabourSector, self).fields_view_get(view_id=view_id,
                                                       view_type=view_type,
                                                       toolbar=toolbar,
                                                       submenu=submenu)
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        res = super(LabourSector, self).search_read(domain, fields, offset, limit, order)
        return res
    ...
    def create(self, vals_list):
        ...
        # Add traces
        model = self.env['ir.model'].search([('model', '=', 'nomenclators.labour_sector')])

        self.env['security.traces'].create({
            'register_time': datetime.now(),
            'user': user.name,
            'model': model.id,
            'description': 'Creación de sector laboral satisfactoria.'
        })

        return super(LabourSector, self).create(vals_list)
    ...
    def write(self, vals):
        ...
        self.env['security.traces'].create({
            ...
            'user': user.name,
            'model': model.id,
            'description': 'Edición de sector laboral satisfactoria.'
        })
        return super(LabourSector, self).write(vals)
    ...
    def unlink(self):
        ...
        #     inscription_model = self.env['professional_registers.inscriptions_help'].search(
        #         [('profession', '=', int(rec.id))])
        ...
        model = self.env['ir.model'].search([('model', '=', 'nomenclators.labour_sector')])
        user = self.env['res.users'].search([('id', '=', int(self.env.uid))])
        msg = 'Eliminación de sector laboral satisfactoria.'
        if count > 1:
        ...
        return super(LabourSector, self).unlink()
    ...
    def _check_name(self):
        for record in self:
            if record.name:
                ...
                # Validar que no comience con espacios
