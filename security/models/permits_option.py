...
class PermitsOption(models.Model):
    _name = "security.permits_option"
    _description = "Permisos por Opciones"
    _rec_name = "user"
    ...
    def _get_domain_user(self):
        ...
        users = self.env["res.users"].search(
            [("company_id", "in", user_login.company_ids.ids)]
        )
        for us in users:
            arra_ids.append(us.id)
    ...
    _sql_constraints = [
        (
            "user_opt_uniq",
            "unique (user, options)",
            "La combinación de usuario y opción seleccionada se encuentra registrada en el sistema!",
        )
    ]

    @api.model
    def fields_get(self, fields=None, attributes=None):
        res = super(PermitsOption, self).fields_get(fields, attributes=attributes)
    ...
    def fields_view_get(
        ...
        res = super(PermitsOption, self).fields_view_get(
        ...
        if not self.env["res.users"].has_group("base.group_erp_manager"):
            ...
            if not self.env["res.users"].has_group("security.permits_access"):
                raise exceptions.ValidationError(
                    "Usted no tiene permisos para visualizar permisos por opciones!"
                )
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        if not self.env["res.users"].has_group("base.group_erp_manager"):
            ...
            option = (
                ...
                .sudo()
                .search([("model", "=", "security.permits_option")])
            )
        ...
        res = super(PermitsOption, self).search_read(
    ...
    def create(self, vals_list):
        ...
        # Add traces
        model_conciliation = self.env['ir.model'].search([('model', '=', 'security.permits_option')])
        user = self.env['res.users'].search([('id', '=', int(self.env.uid))])
        self.env['security.traces'].create({
            'register_time': datetime.now(),
            'user': user.name,
            'model': model_conciliation.id,
            'description': 'Creación de permisos por opciones satisfactoria'
        })

        return super(PermitsOption, self).create(vals_list)
    ...
    def write(self, vals):
        ...
        self.env['security.traces'].create({
            ...
            'user': user.name,
            'model': model_conciliation.id,
            'description': 'Edición de permisos por opciones satisfactoria'
        })

        return super(PermitsOption, self).write(vals)

    def unlink(self):
        model_conciliation = self.env['ir.model'].search([('model', '=', 'security.permits_option')])
        user = self.env['res.users'].search([('id', '=', int(self.env.uid))])
        msg = 'Eliminación de permisos por opciones satisfactoria.'
        ...
        return super(PermitsOption, self).unlink()